...
namespace MyAppFirst.Controllers
{
    public class PedidosController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Pedido pedido, int[] platoIds, int[] cantidades, string comentario)
        {
            var mozoId = HttpContext.Session.GetInt32("UserId");
            if (mozoId.HasValue)
            {
        ...
        public async Task<IActionResult> Edit(int id, string NumeroMesa, int[] platoIds, int[] cantidades, string comentario)
        {
            var pedido = await _context.Pedidos
                                .Include(p => p.Detalles)
                                .FirstOrDefaultAsync(p => p.Id == id);
